...
- Fixed a repeat switch to the SAME third-party provider skipping the CC Switch restart and landing on the login screen (Official → DeepSeek → Official → DeepSeek). The repair was gated on a point-in-time read of `~/.codex/auth.json` that raced with CC Switch's own credential write, so the second switch was misjudged as already consistent and only Codex was restarted. The repair is now edge-triggered: every genuine switch onto a third-party route restarts the bound CC Switch once, bounded by the one-shot per-key latch and the flap circuit breaker rather than by a credential read. The credential check survives only as the fallback that decides whether reloading Codex is safe when CC Switch could not be repaired. (Windows and macOS.)
