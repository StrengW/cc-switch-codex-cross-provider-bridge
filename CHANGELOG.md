...
- README and README.en now state plainly that a normal first run needs no internet access and say when a download happens instead. The mirror order, the pinned-digest rule, the log location, and the install-Python-3.10+-yourself alternative are documented in `docs/ARCHITECTURE.md` under Runtime preparation and in the Chinese user guide.

### Docs

- Both READMEs were rewritten as user-facing documentation and cut by about a third (371 lines to 246, and 370 to 252). The detail that made them read like a technical specification moved into a new Chinese user guide, `docs/USER_GUIDE.zh-CN.md`, and into `docs/ARCHITECTURE.md`, which gained a Runtime preparation section and now also explains why a third-party switch can leave Codex on the login screen and what to do when the automatic repair did not complete. Statements the READMEs repeated five to seven times — the CC Switch boundary, the rule against reviving it in the background, the single controlled-restart exception, the macOS approval steps, the bundled runtime — now appear once, with a link where the reasoning is wanted.
- The macOS Release package now verifies every document it stages rather than two of the five, and a contract test fails when a file under `docs/` is missing from either release package or is not reachable from a README. Both package lists are hard-coded on different platforms and nothing cross-checked them, so a newly added document could ship as a dead link inside the ZIP — the copy users actually read offline.
